...
public class RecursiveBestFirstSearch extends NodeExpander implements Search {
	...
	private SearchResult rbfs(Problem p, Node n, double node_f, double fLimit,
		...
		//Create temporary node in case of removal
		List<Node> nodesToRemove = new ArrayList<Node>(); //Used to remove frontier nodes and unsolvable nodes
		for (Node tempNode : successors) //Test if node violates any of the above conditions
		{
			if (!n.isRootNode()) //Make sure it is not root
			{
				boolean equiv = true; //If node is equivalent to parent
				PuzzleSolver.Board childStates = (PuzzleSolver.Board) tempNode.getState();
				PuzzleSolver.Board parentStates = (PuzzleSolver.Board) n.getParent().getState();

				for (int i = 0; i < childStates.getState().size(); i++)
				{
					if (childStates.getState().get(i) != parentStates.getState().get(i))
					{
						equiv = false; //False if not equivalent to parent state
					}
				}
				if (equiv) 
				{
					nodesToRemove.add(tempNode); //Remove parent node from successor set
				}
			}
		}
		//Remove nodes from successor set that violated above rules if they exist
		if (nodesToRemove.size() != 0) for(Node tempNode : nodesToRemove) successors.remove(tempNode);
		// if successors is empty then return failure, infinity
		...
		for (int s = 0; s < size; s++) {
			// s.f <- max(s.g + s.h, node.f)
			//Overridden, we want the lowest heuristc function
			f[s] = evaluationFunction.f(successors.get(s));
			f[s] += (recursiveDepth + 1);
		}
		...
		while (true) {
			...
			//Only use altIndex if the size of the successors is greater than 1, otherwise we resort to
			//The regular f_limit
			double altFlimit = f.length > 1 ? f[altIndex] : INFINITY;
			// result, best.f <- RBFS(problem, best, min(f_limit, alternative))
			SearchResult sr = rbfs(p, successors.get(bestIndex), f[bestIndex],
					Math.min(fLimit, altFlimit), recursiveDepth + 1);
	...
	private boolean isSolvable(Node state)
	{
		int value = 0;
		PuzzleSolver.Board board = (PuzzleSolver.Board) state.getState();
		for (int i = 0; i < board.getState().size(); i++)
		{
			int temp = board.getState().get(i);
			for (int j = i + 1; j < board.getState().size(); j++)
			{
				if (temp != 0 && board.getState().get(j) != 0 && temp > board.getState().get(j)) value++;
			}
		}
		return (value % 2 == 0);
	}
